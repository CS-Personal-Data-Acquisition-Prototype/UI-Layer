...
impl SessionDisplay {
    ...
    fn show_session_data(&mut self, ui: &mut eframe::egui::Ui) -> () {
        ...
        if ui.button("New Session").clicked() {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                let (status, _val) = session::create_session(&client, &id, account_id).await;
    ...
    fn load_sessions(&mut self) {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            let (status, val) = session::view_sessions_by_user(&client, "session1", &account_id).await;
